  ...
  <div class="card">
    ...
    <form id="employeeForm">
      ...
      <div class="field"><label>Password</label>
        <input type="password" name="emp_password" placeholder="Set a strong password" required minlength="6"></div>
      <button type="button" class="btn" onclick="addEmployee()">Register Employee</button>
    </form>
...
async function addEmployee() {
  ...
  const r = await fetch('/api/add-employee', { method:'POST',
    headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ name:d.emp_name, email:d.emp_email, password:d.emp_password }) });
...
def api_reset_employee_password():
    ...
    if not emp_id or not password:
        return jsonify({"message": "Employee ID and new password are required."})
    try:
